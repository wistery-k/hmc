...
let rec show = function
  | Id id -> "_"
  | T (label, []) -> label
  | T (label, children) -> "(" ^ label ^ " " ^ (String.join " " (List.map show children)) ^ ")"
  | App (fn, args) -> "_"
...
let rec valid' env (tree, q) arglst = (* envとarglstの下で, treeがqという型を持つかどうか *)
  match tree with
    | Id id -> 
      ...
      List.mem (P(q)) typs
    | T (label, children) -> 
      List.exists (fun (s, l, crt) -> s = q && l = label && List.for_all2 (fun c ct -> valid' env (c, ct) arglst) children crt) d
    | App (fn, trees) -> 
...
let valid env (fn, atyp) =  (* envの下で, fnがatypという型を持つかどうか *)
  valid' env (r.(fn), get_ret atyp) (Array.of_list (get_args atyp))
...
let check () =
  let g_max = List.flatten (List.map2 (fun i -> List.map (fun typ -> (i, typ))) (iota size) (List.map atypes (Array.to_list n)))
  in
...
let () =
  print_endline (if check () then "safe!" else "unsafe!")
